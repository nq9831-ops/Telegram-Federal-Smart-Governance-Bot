...
public class ApiAuthAspect {
    ...
    /**
     * 检查当前用户是否拥有指定角色或更高权限。
     * 角色等级：USER(0) < REVIEWER(1) < MODERATOR(2) < ADMIN(3)
     * 例如 checkRole("MODERATOR") 会同时匹配 MODERATOR 和 ADMIN。
     * @param requiredRole 最低要求的角色名
     * @return true 如果当前用户满足角色要求
     */
    public boolean checkRole(String requiredRole) {
        try {
            HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
            String headerRole = request.getHeader("X-User-Role");
            if (headerRole == null || headerRole.isBlank()) return false;
            return getRoleLevel(headerRole) >= getRoleLevel(requiredRole);
        } catch (Exception e) {
            log.debug("checkRole failed: {}", e.getMessage());
            return false;
        }
    }
