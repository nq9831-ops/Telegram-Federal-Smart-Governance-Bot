...
public class AdminHandler implements BotHandler {
    ...
    @Value("${admin.broadcast.daily-limit:2}")
    private int broadcastDailyLimit;
    ...
    private void handleUnban(TelegramBot bot, Long chatId, String text) {
        ...
        try {
            // 广播到所有活跃群组
            var groups = em.createQuery("SELECT g.groupId FROM GroupEntity g WHERE g.isActive = true", Long.class)
                .getResultList();
            ...
            // 私聊通知所有未退订用户
            var users = em.createQuery("SELECT u.userId FROM UserEntity u WHERE u.optOutBroadcast = false", Long.class)
                .setMaxResults(500)
                .getResultList();
            for (var uid : users) {
                try {
                    ...
                    log.debug("Broadcast failed to user {}: {}", uid, e.getMessage());
                }
            }
