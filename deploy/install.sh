...
write_config() {
    info "写入配置到 /etc/tg-federal-bot/..."
    mkdir -p /etc/tg-federal-bot

    # 安全凭据（写入 application.properties，不提交 Git）
...
# ===== 内容审核 =====
moderation.provider=${MODERATION_PROVIDER:-cloud}
moderation.local.api-url=${LOCAL_OLLAMA_URL:-http://localhost:11434}/v1/chat/completions
moderation.local.model=${LOCAL_MODEL:-qwen2.5:7b}
...
setup_service() {
    ...
    # 创建用户
    id tgf &>/dev/null || useradd -r -s /bin/false -d "${DEPLOY_DIR}" tgf
...
ExecStart=/usr/bin/java \
    -Dspring.config.additional-location=/etc/tg-federal-bot/application.properties \
    -jar ${DEPLOY_DIR}/target/${JAR_NAME}
Restart=always
RestartSec=10
LimitNOFILE=65536
Environment=SPRING_CONFIG_ADDITIONAL_LOCATION=/etc/tg-federal-bot/application.properties
...
SVC
    ...
    echo "║ 管理页面:   http://服务器IP:8080/                 ║"
    echo "╠════════════════════════════════════════════════════╣"
    echo "║ 安全配置:   /etc/tg-federal-bot/                  ║"
    echo "║             application.properties (凭据私密)     ║"
    echo "║             application-public.properties (公共)   ║"
    echo "╠════════════════════════════════════════════════════╣"
    echo "║ SQL迁移:    Flyway 首次启动自动执行               ║"
    echo "║ Webhook:    部署后手动设置(见 README/3.3)         ║"
    echo "╚════════════════════════════════════════════════════╝"
    echo ""
}
